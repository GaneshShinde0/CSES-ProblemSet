...
public class RevisitTrafficLights {
    ...
    public static void main(String[] args) throws IOException {
        ...
        long res = 0;
        for(int i=0;i<n;i++){
            Map.Entry<Integer, Integer> higher = treeMap.higherEntry(arr[i]);
            if(higher==null){
                res++;
            }else{
                int count = higher.getValue();
                if(count == 1) treeMap.remove(higher.getKey());
                else treeMap.put(arr[i], count-1);
            }
